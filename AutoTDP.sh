...
set_mcu_powersave() {
    ...
    for path in "${MCU_POWERSAVE_PATHS[@]}"; do
        if [[ -w "$path" ]]; then
            #it looks like it's breaking SMU control on awake, but im unsure
...
monitor_and_adjust() {
    ...
    while true; do
        ...
        # The bottleneck sets the load: GPU main, but only if CPU isn't being used by 50% more (average in that case)
        if (( 3 * gpu_usage > 2 * cpu_signal )); then
            load=$gpu_usage
        else
            load=$(( (cpu_signal + gpu_usage) / 2 ))
        fi
